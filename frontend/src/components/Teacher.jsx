...
export function Teacher({ teacher, ...props }) {
  const group = useRef();
  const { scene } = useGLTF(`/models/Ava.glb`);
  useEffect(() => {
  ...
  const { animations } = useGLTF(`/models/animate.glb`);
  const { actions} = useAnimations(animations, group);
  const [animation, setAnimation] = useState("idleStanding");
  ...
  useEffect(() => {
    if (loading) {
      ...
      setAnimation(randInt(0, 1) ? "talking" : "patting");
    } else {
      setAnimation("idleStanding");
    }
    ...
    if (
      ...
      if (
        ...
        setAnimation((animation) =>
          animation === "talking" ? "patting" : "idleStanding"
...
useGLTF.preload(`/models/Ava.glb`);
